...
class PreprocessorCreator(object):
    ...
    def _create_example(self):
        ...
        feature_configs = {
            feat: tf.FixedLenFeature(shape=[], dtype=tf.string) for feat in self.features
        }
...
class Token1DPreprocessorCreator(PreprocessorCreator):
    ...
    def create_word_vectors_from_post(self, raw_post, feature):
        # vocab has only lowercase words
        word2index = self.indices[feature]
    ...
    def preproc(self, post_mappings):
        ...
        # TODO(MB): un-code-golf this line. atm, this helps see the relationship to post_mappings.
        return {x: self.create_word_vectors_from_post(self.reform_raw(post_mappings[x]), x) for x in self.features}

    def create_preprocessed_input(self, tf_example):
        """
        Create a preprocessor chain inside of the tensorflow graph.
        """
        types = {x: tf.int64 for x in self.features}
        return tf.map_fn(
