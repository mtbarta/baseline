...
from baseline.model import Tagger, create_tagger_model, load_tagger_model
from addons.hyper import util
from addons.hyper import rnn_impl
now = datetime.now()
...
class HyperbolicRNNModel(Tagger):
    ...
    def _compute_sentence_level_loss(self):
        ...
        if self.crf_mask:
            ...
            self.inv_mask = tf.cast(tf.equal(self.mask, 0), tf.float64) * tf.constant(-1e4)
    ...
    def create(labels, embeddings, **kwargs):
        ...
        if rnntype == 'rnn':
            ...
            rnnout, state = tf.nn.bidirectional_dynamic_rnn(cell_1, cell_2, embedseq, sequence_length=model.lengths, dtype=tf.float64)
        ...
        #     model.sent_1 = util.tf_log_map_zero(rnnout_bt_x_h, c_val)
        ...
        #     output_ffnn = util.tf_mob_add(ffnn_s1, hyp_b_ff, c_val)
